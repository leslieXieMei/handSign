...
*/
    ...
    (function() {
        function q(b) {
            ...
            var b = /rgb[a]*\((\d+),\s*(\d+),\s*(\d+)/, c = /#([AaBbCcDdEeFf\d]{2})([AaBbCcDdEeFf\d]{2})([AaBbCcDdEeFf\d]{2})/, a;
            j = void 0;
            (j = s.match(b)) ? a = {
                r: parseInt(j[1], 10),
                g: parseInt(j[2], 10),
            ...
            ;
            a && f ? (b = Math.max.apply(null, [a.r, a.g, a.b]),
            ...
            a = {
                ...
                g: a,
                b: a
            }) : a ? (a = Math.max.apply(null, [a.r, a.g, a.b]),
            b = 1,
            127 < a && (b = -1),
            a = Math.round(a + 96 * b),
            a = {
        ...
        function r(b, j, a) {
            ...
            this.$placeholder = $('<p class="pTip">请在此处手签</p>').appendTo(g);
            ...
            var resetCanvasHtml='<div class="tool">'
                            +'      <b class="resetBtn"></b>'
                               +'   <span class="canvas_reset canvas_line">'
                               +'       <b class="bSet lineMin active" _value="2"><i></i></b>'
                               +'       <b class="bSet lineMid" _value="4"><i></i></b>'
                               +'       <b class="bSet lineMax" _value="8"><i></i></b>'
                               +'   </span>'
                               +'   <span class="canvas_reset canvas_color">'
                               +'       <b class="bSet red" _value="red"><i></i></b>'
                               +'       <b class="bSet blue" _value="blue"><i></i></b>'
                               +'       <b class="bSet black active" _value="black"><i></i></b>'
                               +'   </span>'
                               +'</div>';
            var _sigWrap=g.parent();
            if(_sigWrap.find('.tool').size()==0){
                $(resetCanvasHtml).appendTo(g.parent());
                _sigWrap.find('.resetBtn').on('click',function(){
                    var $sigdiv = $(this).closest('.sigWrap').find('.signatureWrap');
                    _sigWrap.find('.active').each(function(){
                        if($(this).parent().hasClass('canvas_line')){
                            jSigObj.lineWidth=parseInt($(this).attr('_value'));
                        }else{
                            jSigObj.color=$(this).attr('_value');
                        }
                    })
                    $sigdiv.html('').jSignature(jSigObj);   
                    $sigdiv.attr('isWrite','').find('.pTip').css('display','block');
                })
                
                _sigWrap.find('.bSet').on('click',function(){
                    var _value=$(this).attr('_value'),
                        _index=$(this).parent().index();
                    jSigObj={};
                    $(this).addClass('active').siblings().removeClass('active');
                    if(_index==1){
                        jSigObj.lineWidth=parseInt(_value);
                        if(_sigWrap.find('.canvas_color').find('.active').size()>0){
                            jSigObj.color=_sigWrap.find('.canvas_color').find('.active').attr('_value');
                        }
                    }else if((_index==2)){
                        jSigObj.color=_value;
                        if(_sigWrap.find('.canvas_line').find('.active').size()>0){
                            jSigObj.lineWidth=_sigWrap.find('.canvas_line').find('.active').attr('_value');
                        }
                    }
                    var $sigdiv = _sigWrap.find('.signatureWrap');
                    $sigdiv.html('').jSignature(jSigObj);   
                })
            }
            ...
            $(window).bind( 'orientationchange', function(e){
                var $sigdiv = $('.signatureWrap').attr('isWrite','');
                jSigObj={};
                $sigdiv.closest('.sigWrap').find('.tool').remove();
                $sigdiv.html('').jSignature();  
            });
            ...
            var g = function(b) {
                ...
                this.drawStartHandler = function(c) {
                    //console.log('开始手签2');
                    jsignatureWrap.attr('isWrite','true').find('.pTip').css('display','none');
                    jSigObj={};
                    c.preventDefault();
                ...
                this.drawMoveHandler = function(j) {
                    j.preventDefault();
                    b.dataEngine.inStroke && (b.dataEngine.addToStroke(g(j)),
                    d.kick())
                }
                ;
                return this
            ...
            .call({}, this)
              ...
              , h = this.canvas
              , j = $(h);
            this.isCanvasEmulator ? (j.bind("mousemove." + e, p),
            j.bind("mouseup." + e, l),
            j.bind("mousedown." + e, i)) : (h.ontouchstart = function(b) {
                console.log('开始手签1');
                jsignatureWrap.attr('isWrite','true').find('.pTip').css('display','none');
                jSigObj={};
            ...
            if ("ratio" === b || "%" === b.split("")[b.length - 1])
                this.eventTokens[w + ".parentresized"] = c.subscribe(w + ".parentresized", function(b, j, a) {
                    return function() {
                        ...
                        if (g !== a) {
                            for (var d in b)
                                b.hasOwnProperty(d) && (c.unsubscribe(b[d]),
                                delete b[d]);
                            var s = n.settings;
        ...
        var e = "jSignature"
          ...
          , u = function(b, a) {
            ...
            c.getLength() <= this.lineCurveThreshold && (c = this.canvasContext,
            f = d.x,
            d = d.y,
            i = g.x,
            g = g.y,
        ...
        r.prototype.initializeCanvas = function(b) {
            ...
            "ratio" === b.height ? c.css("height", "100%") : "ratio" === b.width && c.css("width", Math.round(c.height() * b.sizeRatio));
            c.addClass(e);
            a.width = c.width();
            a.height = c.height();
            b = a;
            if (b.getContext)
                ...
                if (d) {
                    ...
                    if (1 !== d)
                        try {
                            $(b).children("object").get(0).resize(Math.ceil(b.width * d), Math.ceil(b.height * d)),
        ...
        var p = window, h = function(b, a) {
            ...
            if (void 0 === a && ("string" === typeof b && "data:" === b.substr(0, 5)) && (a = b.slice(5).split(",")[0],
        ...
        var e = function(e) {
          ...
          , v = function(e) {
            for (var h = [], e = e.split(""), a = e.length, f, d = 1, c = [], l = 0, i = 0; i < a; i++)
                f = e[i],
                f in q || "Z" === f || "Y" === f ? (0 !== c.length && (c = parseInt(c.join(""), t) * d + l,
                h.push(c),
                l = c),
                "Z" === f ? (d = -1,
                c = []) : "Y" === f ? (d = 1,
    ...
    (function() {
        ...
        function t(a, f, d) {
            ...
            return c.getLength() > d ? (d = 2 < f ? (new m(a.x[f - 3],a.y[f - 3])).getVectorToPoint(i) : new q(0,0),
            a = 0.35 * c.getLength(),
            i = c.angleTo(d.reverse()),
            f = e.angleTo(c.reverse()),
            d = (new q(d.x + c.x,d.y + c.y)).resizeTo(Math.max(0.05, i) * a),
            e = (new q(c.x + e.x,c.y + e.y)).reverse().resizeTo(Math.max(0.05, f) * a),
            e = new q(c.x + e.x,c.y + e.y),
            ["c", k(d.x, 2), k(d.y, 2), k(e.x, 2), k(e.y, 2), k(c.x, 2), k(c.y, 2)]) : ["l", k(c.x, 2), k(c.y, 2)]
        }
        function r(a, f) {
            var d = a.x.length - 1
              , c = new m(a.x[d],a.y[d])
        ...
        function y(a) {
            var f = ['<?xml version="1.0" encoding="UTF-8" standalone="no"?>', '<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">'], d, c = a.length, l, i = [], h = [], k = l = d = 0, n = 0, p = [];
            if (0 !== c) {
                for (d = 0; d < c; d++) {
                    k = a[d];
                    n = [];
            ...
            for (c = p.length; d < c; d++)
                l = p[d],
                f.push('<path fill="none" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" d="' + e(l, k, n) + '"/>');
            f.push("</svg>");
            return f.join("")
        }
        function v(a) {
        ...
        ("undefined" != typeof exports ? exports : u).simplify = function(a, e, d) {
            ...
            l = a;
            var d = l.length, c = new ("undefined" != typeof Uint8Array ? Uint8Array : Array)(d), i = 0, h = d - 1, m, p, b = [], j = [], s = [];
            for (c[i] = c[h] = 1; h; ) {
                n = 0;
                for (k = i + 1; k < h; k++) {
                    m = l[k];
        ...
        if ("function" !== typeof x)
            var x = function(a) {
                var e = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".split(""), d, c, h, i, k = 0, m = 0, n = "", n = [];
                do
                    d = a.charCodeAt(k++),
                    c = a.charCodeAt(k++),
                    h = a.charCodeAt(k++),
